    ...
    for clonotype in adict:
        ...
        if p:
            ...
            if v_gene in TRV_gene and J_gene in TRJ_gene:
                ...
                os.exit("NULL_J")
                # sys.exit("%s\t%s\t%s\t%s\t%s\tNULL_J"%(clonotype, v_gene,cdr3,J_gene,adict[clonotype]['uminumber'], adict[clonotype]['readsNumber']))
            #     print("%s\t%s\t%s\t%s\t%s\tNULL_J"%(clonotype, v_gene,cdr3,J_gene,'\t'.join(other)))
            elif v_gene not in TRV_gene and J_gene in TRJ_gene :
                os.exit("NULL_V")
                # sys.exit("%s\t%s\t%s\t%s\t%s\tNULL_V"%(clonotype, v_gene,cdr3,J_gene,adict[clonotype]['uminumber'], adict[clonotype]['readsNumber']))
                # print("%s\t%s\t%s\t%s\t%s\tNULL_V"%(clonotype, v_gene,cdr3,J_gene,'\t'.join(other)))
            elif v_gene not in TRV_gene and J_gene not in TRJ_gene :
                os.exit("NULL_VJ")
                # sys.exit("%s\t%s\t%s\t%s\t%s\tNULL_VV"%(clonotype, v_gene,cdr3,J_gene,adict[clonotype]['uminumber'], adict[clonotype]['readsNumber']))

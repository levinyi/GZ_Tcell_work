...
path_list = unlist(strsplit(args[1], "/")) #按照/拆分,
...
p1 = ggplot(data,aes(x=readsNumber, y=..count..)) + geom_histogram(stat="count", color="black", fill="white") +
	...
	scale_x_continuous(breaks=seq(0,as.integer(args[2]),10), limits=c(0,as.integer(args[2]))) + # 设置x轴的坐标范围（limits），和间隔（breaks）
	ggtitle(gene_name) + theme(plot.title = element_text(hjust = 0.5))  # 设置标题，并居中放置在图案上方。
ggsave(paste(gene_name,args[2], "umicount.histogram.png",sep="."),p1)

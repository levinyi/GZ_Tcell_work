...
# data integration
 ...
 if (n_projects == 1) {
   ...
   # integrate GEX
   integrated_gex <- integrate_datasets(srt_list = data_list$gex_list,
   ...
   integrated_csp <- NULL
   if (in_list$has_csp) {
     integrated_csp <- integrate_datasets(srt_list = data_list$csp_list,
   ...
   if (in_list$has_vdj) {
     ...
     merged_vdj <- merge_vdj(vdj_list = data_list$vdj_list)
   }
 }
 if (verbose) {
 ...
 if (in_list$has_csp) {
   ...
   integrated_gex <- fuse_cite_seq(integrated_gex, integrated_csp,
                                   integrated = n_projects > 1)
 }
 # cluster GEX, but limit to cells that have CSP
 if (n_projects == 1) {
   integrated_gex <- normalize_and_find_var_feats(
     integrated_gex, normalization_method = "SCT",
     regress = c("pct_mito", "nCount_RNA"), rmv_grep = "TR[AB]V")
 }
 ...
 # generate plots
 temp_var <- batch_plots(integrated_gex, in_list)
 if (verbose) {
 ...
 ExportSeurat(integrated_gex,overwrite=TRUE,

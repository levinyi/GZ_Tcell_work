...
rootpath_jet_scaling <- function(jet_colors = c(ROOTPATH_COLOURS["navy"],
                                                ROOTPATH_COLOURS["blue"],
                                                ROOTPATH_COLOURS["skyblue"],
                                                ROOTPATH_COLOURS["yellow"],
                                                ROOTPATH_COLOURS["orange"],
                                                ROOTPATH_COLOURS["red"]),
                                 value_range=NULL) {
  new_scale <- scale_color_gradientn(colours = jet_colors,
                                     limits = value_range)
  return(new_scale)
}
...
gen_feature_plots <- function(gex_seurat, plot_dir,
                              ...
                              colors = list(
                                          #"low" = "lightgrey",
                                          #"high" = "blue"
                                          "low" = ROOTPATH_COLOURS[["green"]],
                                          "high" = ROOTPATH_COLOURS[["violet"]]
                                        ),
  ...
  drc_genes = c("CCR7", "GZMK", "GZMB", "FASLG", "GZMA", "TCF7", "IL7R",
                ...
                "SCYL3", "CASP3", "CX3CR1", "KLRK1", "CXCL13")
  drc_descriptions = c("Naive/ CM", "TE", "Effector/ TE", "TE", "TE", "CM",
                       ...
                       "Effectors", "TR TIL?", "NeoAg reactive?")
...
#' + jitter plot, with some additional visualization customization support
#'
#' Inputs:
#' @param srt_obj - a Seurat object from which to get the data
#' @param metric - the metadata entry or feature name to plot
#' @param group_by - another metadata or feature name from srt_obj. This is
#' used to stratify the data in metric for a multi-color jitter plot
#' @param title etc - parameters specifying the plot visuals and save location
#' etc.
#' Returns:
#' @param vln_plot - a ggplot2 object of the specified plot
...
#' Convenience function that uses Seurat FeaturePlot to plot multiple genes,
#' while still maintaining some customizability of colors etc
multi_feature_plot <- function(srt_obj, feature_list, ggplot_mods,
                               point_size=0.5) {
  plot_list <- FeaturePlot(srt_obj,

...
for (each_file in args){
    print(paste("input:", each_file, sep=" "))
    a = unlist(strsplit(each_file,"/"))
    sample_name = unlist(strsplit(a[length(a)],"\\."))[2] # Total.G208E1.TRAB.wells.boole.matrix.csv : extract G208E1
    # print(sample_name)
    ...
    if(length(unique(total_data$barcode))>24){
    ...
    if (endsWith(each_file, ".wells.count.matrix.Raw.csv")){
        p = ggplot(total_data, aes(x=barcode, y=data_sum, group=groups,color=groups)) +geom_point() + geom_line() + theme_bw() +
          ...
          common_themes + axis_text_x +
          scale_colour_discrete(breaks=c("TRA","TRB"), labels=c("TRA-Raw", "TRB-Raw"))
        ggsave(paste(sample_name, "read_count_for_each_well.Raw.png", sep = "_"), width = 10, height = 6)
        # png(paste(sample_name, "read_count_for_each_well.Raw.png", sep="_"))
	# print(p)
        print(paste("output: ", sample_name, "read_count_for_each_well.Raw.png",sep="_"))
    }else if (endsWith(each_file, ".wells.boole.matrix.Raw.csv")){
        p = ggplot(total_data, aes(x=barcode,y=data_sum, group=groups, color=groups)) + geom_point() + geom_line() + theme_bw() +
            ...
            axis_text_x +
            scale_colour_discrete(breaks=c("TRA","TRB"), labels=c("TRA-Raw", "TRB-Raw"))
        ggsave(paste(sample_name, "TCRs_count_for_each_well.Raw.png", sep="_"), width = 10, height = 6)
        print(paste("output: ", sample_name, "TCRs_count_for_each_well.Raw.png",sep="_"))
        ...
        p = ggplot(total_data, aes(x=barcode, y=data_sum, group=groups,color=groups)) + geom_point() + geom_line() + theme_bw() +
          ...
          common_themes + axis_text_x +
          scale_colour_discrete(breaks=c("TRA","TRB"), labels=c("TRA-Filtered", "TRB-Filtered"))
        ggsave(paste(sample_name, "read_count_for_each_well.Filtered.png", sep="_"), width = 10, height = 6)
        print(paste("output: ", sample_name, "read_count_for_each_well.Filtered.png", sep="_"))
        ...
        p = ggplot(total_data, aes(x=barcode, y=data_sum, group=groups, color=groups)) + geom_point() + geom_line() + theme_bw() +
          ...
          common_themes + axis_text_x +
          scale_colour_discrete(breaks=c("TRA","TRB"), labels=c("TRA-Filtered", "TRB-Filtered"))
        # ggsave(paste(sample_name, "TCRs_count_for_each_well.Filtered.png",sep="_"), width=10, height=10)
        ggsave(paste(sample_name, "TCRs_count_for_each_well.Filtered.png",sep="_"), width = 10, height = 6)
        print(paste("output: ", sample_name, "TCRs_count_for_each_well.Filtered.png",sep="_"))
    }else{
        print(paste("your input file name are not correct.\nError: Incorrect File Name.\n",sample_name,sep=":"))
    }
}

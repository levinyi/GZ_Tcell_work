...
def main():
    ...
    for each_B_clone in TRB_dict:
        ...
        for each_A_clone in TRA_dict:
            ...
            shared_wells_dict.setdefault(each_B_clone, {})[each_A_clone] = Wab
            if W >= Wab > 0:
                p = p_value_dict[Wb][Wa][Wab]
                clonotype2p_dict.setdefault(each_B_clone, {})[each_A_clone] = p

    print("TRB_cloneId\tTRB_count\tTRB_clone_wells\tTRA_cloneId\tTRA_count\tTRA_clone_wells\tShared_Wells\tmin_P-value\tratio")
    for each_B_clone in clonotype2p_dict:
        # modified by author at 20200317.
        candidate_keys = []
        for each in sorted(clonotype2p_dict[each_B_clone].items(),key=lambda item : item[1], reverse=False):
            ratio = count_dict[each_B_clone] / float(count_dict[each[0]])
        ...
        # min_value = min(clonotype2p_dict[each_B_clone].values())
        # min_keys = [k for k in clonotype2p_dict[each_B_clone] if clonotype2p_dict[each_B_clone][k] == min_value]
        for each_can in candidate_keys:
            print("{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format(
                each_B_clone,
                count_dict[each_B_clone],
                clonotype2wells_dict[each_B_clone],
                each_can[0],
                count_dict[each_can[0]],
                clonotype2wells_dict[each_can[0]],
                shared_wells_dict[each_B_clone][each_can[0]],
                each_can[1],
                count_dict[each_B_clone]/float(count_dict[each_can[0]]),)
            )
'''
        for each_tra in min_keys:
            print("{}\t{}\t{}\t{}\t{}\t{}".format(
                each_B_clone,
                clonotype2wells_dict[each_B_clone],
                each_tra,
                clonotype2wells_dict[each_tra],
                shared_wells_dict[each_B_clone][each_tra],
                min_value,)
            )
'''
